...
export const ButtonNextStage = () => {
    ...
    const payOperation = () => {
      ...
      if (stateInfo.balance.method === 'USD') {
        if (stateInfo.balance.usd >= stateInfo.products.totalPriceCart) {
          localStorage.setItem('balance', JSON.stringify({ usd: Number((balanceLocal.usd - stateInfo.products.totalPriceCart).toFixed(2)), coins: balanceLocal.coins }))
        }
      } else if (stateInfo.balance.method === 'Coins') {
        if (stateInfo.balance.usd >= stateInfo.products.totalPriceCart) {
          localStorage.setItem('balance', JSON.stringify({ usd: balanceLocal.usd, coins: Number((balanceLocal.coins - stateInfo.products.totalPriceCart).toFixed(2)) }))
        }
      }
    }
    await payOperation()
  ...
  return (
    <button onClick={location === '/cart/pay' ? onPay : onClickHandler} className='border rounded border-green-700 w-52 h-16 bg-green-500 hover:bg-green-600 text-white text-2xl'>
      {textButton()}
    </button>
  )
}
